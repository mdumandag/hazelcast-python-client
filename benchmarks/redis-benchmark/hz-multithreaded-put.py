...
def do_benchmark():
    class ClientThread(threading.Thread):
        ...
        def run(self):
            m = self.client.get_map("map").blocking()
            while True:
                key = key_format.format(int(random.random() * ENTRY_COUNT))
                start = time.time_ns()
                m.set(key, value_data)
                end = time.time_ns()
    ...
    threads = [ClientThread("client-thread-%d" % i, config) for i in range(0, THREAD_COUNT)]
    for t in threads:
        t.start()
    ...
    with open("hz-{}t-put-th.txt".format(THREAD_COUNT), "w") as out:
        out.write(str(throughputs))

    with open("hz-{}t-put-hist.txt".format(THREAD_COUNT), "wb") as out:
