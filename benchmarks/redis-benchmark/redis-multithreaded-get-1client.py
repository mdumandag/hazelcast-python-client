...
def do_benchmark():
    ...
    with open("redis-{}t-get-th-1client.txt".format(THREAD_COUNT), "w") as out:
        out.write(str(throughputs))

    with open("redis-{}t-get-hist-1client.txt".format(THREAD_COUNT), "wb") as out:
...
def fill_map():
    for i in range(ENTRY_COUNT):
        client.set(key_format.format(i), value_data)
