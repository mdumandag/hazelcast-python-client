...
def handle(message, handle_map_partition_lost_event=None):
    message_type = message.get_message_type()
    if message_type == _EVENT_MAP_PARTITION_LOST_MESSAGE_TYPE and handle_map_partition_lost_event is not None:
        initial_frame = message.next_frame()
        buf = initial_frame.buf
        partition_id = FixSizedTypesCodec.decode_int(buf, _EVENT_MAP_PARTITION_LOST_PARTITION_ID_OFFSET)
        uuid = FixSizedTypesCodec.decode_uuid(buf, _EVENT_MAP_PARTITION_LOST_UUID_OFFSET)
