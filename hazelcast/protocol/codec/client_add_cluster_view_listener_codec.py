...
from hazelcast.protocol.client_message import ClientMessage, REQUEST_HEADER_SIZE, create_initial_frame, EVENT_HEADER_SIZE
...
def handle(message, handle_members_view_event=None, handle_partitions_view_event=None):
    message_type = message.get_message_type()
    if message_type == _EVENT_MEMBERS_VIEW_MESSAGE_TYPE and handle_members_view_event is not None:
        initial_frame = message.next_frame()
        buf = initial_frame.buf
        version = FixSizedTypesCodec.decode_int(buf, _EVENT_MEMBERS_VIEW_VERSION_OFFSET)
        member_infos = ListMultiFrameCodec.decode(message, MemberInfoCodec.decode)
    ...
    if message_type == _EVENT_PARTITIONS_VIEW_MESSAGE_TYPE and handle_partitions_view_event is not None:
        initial_frame = message.next_frame()
        buf = initial_frame.buf
        version = FixSizedTypesCodec.decode_int(buf, _EVENT_PARTITIONS_VIEW_VERSION_OFFSET)
        partitions = EntryListUUIDListIntegerCodec.decode(message)
