...
class _BitmapIndexOptions(object):
...
class _IndexConfig(object):
    ...
    def __init__(self, name=None, type=None, attributes=None, bitmap_index_options=None):
        ...
        self._bitmap_index_options = _BitmapIndexOptions()
    ...
    def bitmap_index_options(self, value):
        if isinstance(value, dict):
            self._bitmap_index_options = _BitmapIndexOptions.from_dict(value)
        elif isinstance(value, _BitmapIndexOptions):
...
class _IndexUtil(object):
    ...
    def validate_and_normalize(map_name, index_config):
        ...
        if len(original_attributes) > _IndexUtil._MAX_ATTRIBUTES:
            raise ValueError(
                "Index cannot have more than %s attributes %s" % (_IndexUtil._MAX_ATTRIBUTES, index_config))
